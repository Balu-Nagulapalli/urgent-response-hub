...
import { getIncidents, ServiceNowIncidentRecord } from "@/services/api";
...
const FIELDS = [
  ...
  "u_full_address","u_gps_latitude","u_gps_longitude","opened_at","caller_id",
].join(",");
...
const AdminDashboard = () => {
  ...
  const fetchIncidents = useCallback(async () => {
    ...
    try {
      ...
      const { data: json } = await getIncidents<ServiceNowIncidentRecord[]>({
        sysparm_query: query,
        sysparm_fields: FIELDS,
        sysparm_limit: 50,
        sysparm_display_value: true,
      });
      setIncidents((json?.result ?? []) as SNIncident[]);
  ...
  const formatTime = (str: string) => {
    if (!str) return "—";
    const utc = str.includes("T") ? str : str.replace(" ", "T") + "Z";
        ...
        <div className="container max-w-7xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Link to="/"><Button variant="ghost" size="sm" className="gap-1 text-muted-foreground -ml-2"><ArrowLeft className="h-4 w-4" /> Home</Button></Link>
              </div>
              <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
                <ShieldAlert className="h-6 w-6 text-primary" /> Incident Dashboard
              </h1>
            ...
            <div className="flex gap-2">
              ...
              <Link to="/report">
                <Button className="gap-2"><AlertTriangle className="h-4 w-4" /> New Report</Button>
              </Link>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
              ...
              <div className="flex gap-1">
                {(["app", "all"] as const).map((s) => (
                  <button key={s} onClick={() => setSource(s)}
                    className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${source === s ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-muted-foreground hover:border-primary/50"}`}>
                    {s === "app" ? "App Reports" : "All Incidents"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-1 flex-wrap">
              {([["all","All States"],["1","New"],["2","In Progress"],["3","On Hold"],["6","Resolved"],["7","Closed"]] as [FilterState, string][]).map(([val, lbl]) => (
                <button key={val} onClick={() => setStateFilter(val)}
                  className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${stateFilter === val ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-muted-foreground hover:border-primary/50"}`}>
                  {lbl}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-1 flex-wrap">
              {([["all","All Types"],["medical","🏥 Medical"],["rescue","🚨 Rescue"],["fire","🔥 Fire"],["police","👮 Police"],["others","📝 Others"]] as [CategoryFilter, string][]).map(([val, lbl]) => (
                <button key={val} onClick={() => setCatFilter(val)}
                  className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${catFilter === val ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-muted-foreground hover:border-primary/50"}`}>
                  {lbl}
                </button>
              ))}
            </div>
          ...
          {loading && (
            <div className="flex items-center justify-center py-20 gap-3 text-muted-foreground">
              <Loader2 className="h-6 w-6 animate-spin" /> Fetching incidents from ServiceNow…
            </div>
          )}
          ...
          {!loading && filtered.length > 0 && (
            <div className="flex gap-6 items-start">
              ...
              <div className={`flex-1 min-w-0 space-y-3 ${selected ? "hidden lg:block" : ""}`}>
                <p className="text-sm text-muted-foreground mb-3">Showing {filtered.length} of {incidents.length} incidents</p>
                {filtered.map((inc) => {
                  ...
                  return (
                    <div key={inc.sys_id} onClick={() => setSelected(isSelected ? null : inc)}
                      className={`bg-card border rounded-xl p-4 cursor-pointer transition-all hover:shadow-md ${isSelected ? "border-primary shadow-md ring-1 ring-primary/20" : "border-border hover:border-primary/30"}`}>
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-mono text-sm font-semibold text-foreground">{inc.number}</span>
                          <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${cat.color}`}>{cat.emoji} {cat.label}</span>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${state.color}`}>{state.label}</span>
                        </div>
                      </div>
                      <p className="text-sm text-foreground font-medium mb-2 line-clamp-1">{inc.short_description}</p>
                      <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                        {inc.u_caller_email && <span className="flex items-center gap-1"><Mail className="h-3 w-3" /> {inc.u_caller_email}</span>}
                        {inc.u_caller_phone && <span className="flex items-center gap-1"><Phone className="h-3 w-3" /> {inc.u_caller_phone}</span>}
                        {inc.u_full_address && <span className="flex items-center gap-1 truncate max-w-[260px]"><MapPin className="h-3 w-3 shrink-0" /> {inc.u_full_address.split(",")[0]}</span>}
              ...
              {selected && (
                <div className="w-full lg:w-96 shrink-0 bg-card border border-border rounded-xl overflow-hidden sticky top-6">
                  <div className="flex items-center justify-between p-4 border-b border-border">
                    ...
                    <button onClick={() => setSelected(null)} className="text-muted-foreground hover:text-foreground p-1 rounded-lg hover:bg-muted">✕</button>
                  </div>

                  <div className="p-4 space-y-4 max-h-[75vh] overflow-y-auto">
                    <div className="flex gap-2 flex-wrap">
                      <span className={`text-xs px-2 py-1 rounded-full border font-medium ${(EMERGENCY_LABELS[selected.category] ?? EMERGENCY_LABELS.others).color}`}>
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${(STATE_CONFIG[selected.state] ?? STATE_CONFIG["1"]).color}`}>
                        {(STATE_CONFIG[selected.state] ?? STATE_CONFIG["1"]).label}
                      </span>
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${(PRIORITY_CONFIG[selected.priority] ?? PRIORITY_CONFIG["3"]).color}`}>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 text-sm"><Mail className="h-3.5 w-3.5 text-muted-foreground shrink-0" /><span>{selected.u_caller_email || "—"}</span></div>
                        <div className="flex items-center gap-2 text-sm"><Phone className="h-3.5 w-3.5 text-muted-foreground shrink-0" /><span>{selected.u_caller_phone || "—"}</span></div>
                        {callerName(selected) !== "—" && (
                          <div className="flex items-center gap-2 text-sm"><AlertTriangle className="h-3.5 w-3.5 text-muted-foreground shrink-0" /><span>{callerName(selected)}</span></div>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-start gap-2 text-sm"><MapPin className="h-3.5 w-3.5 text-muted-foreground shrink-0 mt-0.5" /><span>{selected.u_full_address || "—"}</span></div>
                        {selected.u_gps_latitude && (
                          <>
                            <div className="bg-muted rounded-lg px-3 py-2 text-xs text-muted-foreground font-mono">GPS: {selected.u_gps_latitude}, {selected.u_gps_longitude}</div>
                            <a href={`https://www.google.com/maps?q=${selected.u_gps_latitude},${selected.u_gps_longitude}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline flex items-center gap-1">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-foreground whitespace-pre-wrap bg-muted rounded-lg p-3">{selected.description || selected.short_description || "—"}</p>
                    </div>

                    <a href={`${SN_INSTANCE()}/nav/ui/classic/params/target/incident.do%3Fsys_id%3D${selected.sys_id}`} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" className="w-full gap-2 mt-2">Open in ServiceNow ↗</Button>
                    </a>
                  </div>
                </div>
              )}
