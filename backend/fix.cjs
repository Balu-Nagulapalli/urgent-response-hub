...
app.post("/speech-to-text", upload.single("file"), async (req, res) => {
  try {
    ...
    console.log("🎙️ Sending to Groq API...");
    ...
    if (req.file) fs.unlink(req.file.path, () => {});
    res.status(500).json({ error: "Error processing audio", details: err.response?.data?.error?.message || err.message });
  }
});
...
console.log('✅ server.js updated with FormData support');
